...
declare type GroupRule = String & { [isOpen]?: true | false }

declare type Selectors = string[] & { [isOpen]?: true | false }
...
const createGetComputedCss = (
	...
	const getComputedCss = (
		...
		const groupRules = openConditions.map(Object) as GroupRule[]
					...
					if (name === 'when') {
						for (const conditionName in data) {
							...
							if (conditionName in conditions) {
								processNestedGroup(true, conditions[conditionName], data[conditionName] as Styles)
							} else {
								processNestedGroup(true, conditionName, data[conditionName] as Styles)
							}
						}
					} else {
