...
const assign = <T, U>(target: T, source: U): T & U =>
...
const createCss: StyledSheetFactory = function createCss(init?: StyledSheetFactoryInit) {
	...
	const sheet: StyledSheet = assign(
		function css(...inits: StyledSheetInit[]) {
			...
			const rule: StyledRule = assign(
				function Rule(init?: StyledRuleInit) {
					// variants to express
